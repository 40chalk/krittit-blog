...
import { seriesOnOrOff } from '../../global/site-settings-and-info'
...
export function getLatestSeriesPost(seriesIdentifier) {
  const posts = getAllPost()
  const seriesPosts = posts.map((post) => {
    if (post.series !== seriesIdentifier) {
      return
    }
  ...
  const sorted = sortSeriesPostsByLatest(seriesPosts)
